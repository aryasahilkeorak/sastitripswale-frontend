...
export default function CompleteProfile() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!form.fullName.trim()) return toast('⚠️', 'Please enter your full name');
    if (!form.city.trim()) return toast('⚠️', 'Please enter your city');
    if (!form.gender) return toast('⚠️', 'Please select your gender');
    if (interests.length === 0) return toast('⚠️', 'Pick at least one travel interest');
    if (form.hasVehicle && !form.vehicleType) return toast('⚠️', 'Select your vehicle type');
    if (!aadhaarFile) return toast('⚠️', 'Aadhaar document is required for verification');
    ...
    try {
      ...
      setUser(data.user);
      toast('✅', 'Profile complete! You can now plan and join trips.');
    ...
    <section className="cp-section">
      <div className="container" style={{ maxWidth: 640 }}>
        <div className="text-center mb-4">
          <div className="section-tag" style={{ margin: '0 auto 12px' }}><i className="ri-user-settings-fill" /> Almost there</div>
        ...
        <form className="card cp-form" onSubmit={submit}>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <div className="upload-box" onClick={() => document.getElementById('cp-aadhaar')?.click()}>
                <div className="upload-label">{aadhaarFile ? <><i className="ri-check-line" style={{ color: 'var(--fire)' }} /> {aadhaarFile.name}</> : 'Upload Aadhaar'}</div>
            ...
            <div className="form-group">
              ...
              <div className="upload-box" onClick={() => document.getElementById('cp-pan')?.click()}>
                <div className="upload-label">{panFile ? <><i className="ri-check-line" style={{ color: 'var(--fire)' }} /> {panFile.name}</> : 'Upload PAN'}</div>
          ...
          <button className="btn btn-primary btn-lg" style={{ width: '100%', justifyContent: 'center' }} disabled={busy}>
            {busy ? <span className="spinner" /> : <i className="ri-check-double-line" />} Save &amp; Finish
          </button>
        </form>
      </div>
    </section>
