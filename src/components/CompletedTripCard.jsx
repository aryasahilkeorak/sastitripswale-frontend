    ...
    <Link to={`/trips/${trip._id}`} className="card trip-card fade-up" style={{ color: 'inherit' }}>
      <div className="trip-card-img-wrap">
        ...
        <div style={{ position: 'absolute', top: 14, left: 14, display: 'flex', gap: 6 }}>
          ...
          {days && <span className="badge badge-gold">{t('completedTripCard.daysBadge').replace('{days}', days)}</span>}
          <span className="badge badge-green"><i className="fa-solid fa-trophy" /> {t('completedTripCard.completed')}</span>
        </div>
      </div>
      ...
      <div className="trip-card-body">
        ...
        {trip.organizer && (
          <p style={{ color: 'var(--text-3)', fontSize: '0.78rem', marginBottom: 12 }}>
            <i className="fa-solid fa-user" /> {t('completedTripCard.hostedBy').split('{name}')[0]}
            <strong style={{ color: 'var(--text)', fontWeight: 700 }}>{trip.organizer.username || trip.organizer.fullName}</strong>
            {t('completedTripCard.hostedBy').split('{name}')[1]}
          </p>
        )}
        <div style={{ display: 'flex', gap: 16, fontSize: '0.78rem', color: 'var(--text-3)', marginBottom: 14, flexWrap: 'wrap' }}>
          ...
          <span>
            <i className="fa-solid fa-users" /> {t('completedTripCard.travelers').replace('{count}', trip.filledSeats || 0)}
          </span>
        </div>
        <div
          style={{
        ...
        >
          <div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-3)' }}>{t('completedTripCard.actualCostPerHead')}</div>
            <div className="trip-price">{rupee(perHead)}</div>
          </div>
          <span className="btn btn-sm btn-primary">{t('completedTripCard.viewRecap')}</span>
        </div>
      </div>
    </Link>
  );
